...
/* ──────────────────────────────────────────────────────────
   PAGE DATA — change only this object to reuse the layout
   below for any other framework (CMMI, GDPR, HIPAA, etc.)
   ────────────────────────────────────────────────────────── */
const data = {
  code: 'CMMC',
  title: 'Cybersecurity Maturity Model Certification',
  authority: 'Department of Defense (DoD)',
  authorityUrl: 'https://dodcio.defense.gov/CMMC/',
  year: '2020',
  description: 'CMMC is a tiered cybersecurity verification standard that confirms — rather than simply assumes — that contractors and subcontractors in the Defense Industrial Base (DIB) are properly protecting Federal Contract Information (FCI) and Controlled Unclassified Information (CUI).',

  whatIs: 'CMMC is a unified cybersecurity standard for current and future DoD contractors, designed to ensure appropriate cybersecurity practices and processes are in place to protect sensitive defense information across the supply chain.',
  roleImportance: 'It standardizes cybersecurity requirements across the entire DoD supply chain, moving the program away from self-certification toward independent, third-party verified assessments for contractors handling more sensitive data.',

  benefits: [
    'Eligibility to bid on and win DoD contracts',
    'Stronger protection against advanced persistent threats (APTs)',
    'Improved overall cybersecurity posture and incident readiness',
    'Standardized security expectations across the supply chain',
    'A clear competitive advantage when bidding in the defense sector',
  ],
  rules: [
    'Implement controls based on NIST SP 800-171 (and 800-172 for Level 3)',
    'Undergo third-party C3PAO assessment for Level 2 and above',
    'Maintain continuous compliance with annual SPRS affirmations',
    'Flow down the same CMMC level requirement to subcontractors',
    'Protect CUI and FCI across every in-scope system, not just the network perimeter',
  ],

  whoMustComply: [
    'DoD prime contractors',
    'DoD subcontractors at every tier',
    'Suppliers in the Defense Industrial Base (DIB)',
    'Any organization that processes, stores, or transmits FCI or CUI',
  ],
  complianceRequirements: [
    ...
    'Security assessment, authorization, and SPRS reporting',
  ],

  penalties: 'Without a current CMMC level recorded in SPRS, a company cannot be awarded a new DoD contract, cannot have a contract option exercised, and risks termination of work already underway — effectively locking organizations out of the defense supply chain. Misrepresenting compliance can also trigger False Claims Act liability.',
  ...
  howWeSupport: [
    ...
    'Preparation and evidence packaging for C3PAO audits',
    'Automated compliance reporting and evidence collection',
    'Subcontractor flow-down policy templates',
  ],
};
...
const GlobalStyles = () => (
  <style>{`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .info-card {
      border-radius: 18px; padding: 28px 26px; background: rgba(255,255,255,0.85);
      border: 1px solid rgba(48,111,116,0.15); transition: all 0.3s ease;
    }
    .info-card:hover { transform: translateY(-3px); border-color: rgba(48,111,116,0.4); box-shadow: 0 14px 32px rgba(48,111,116,0.12); }

    .cta-primary {
      display:inline-flex; align-items:center; gap:10px; padding:14px 30px; border-radius:50px;
      background: linear-gradient(135deg, #306F74, #74C3BA); color:#fff; font-weight:700; font-size:14px;
      text-decoration:none; transition: all 0.3s ease; box-shadow: 0 8px 22px rgba(48,111,116,0.4);
    }
    .cta-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(48,111,116,0.55); }
    .cta-secondary {
      display:inline-flex; align-items:center; gap:10px; padding:14px 30px; border-radius:50px;
      background: rgba(255,255,255,0.06); color:#fff; font-weight:700; font-size:14px;
      text-decoration:none; border:1px solid rgba(255,255,255,0.3); transition: all 0.3s ease;
    }
    .cta-secondary:hover { background: rgba(255,255,255,0.14); border-color:#74C3BA; transform: translateY(-2px); }
  `}</style>
);

/* ──────────────────────────────────────────────────────────
   NAVBAR — identical to CyberRegulations.jsx
   ────────────────────────────────────────────────────────── */
const Navbar = () => (
...
/* ──────────────────────────────────────────────────────────
   HERO — dark, centered, in the site's teal palette
   ────────────────────────────────────────────────────────── */
const Hero = () => (
  <section style={{ marginTop:'90px', position:'relative', overflow:'hidden', fontFamily:SF, background:'linear-gradient(180deg, #08191c 0%, #0c2326 60%, #0a1f22 100%)', padding:'90px 30px 80px' }}>
    ...
    <div style={{ position:'relative', zIndex:2, maxWidth:'720px', margin:'0 auto', textAlign:'center', animation:'heroFadeUp 0.6s ease both' }}>
      ...
      <p style={{ color:'rgba(255,255,255,0.55)', fontSize:'13.5px', margin:'0 0 22px' }}>
        Established by <a href={data.authorityUrl} target="_blank" rel="noreferrer" style={{ color:'#cfe9e6', fontWeight:'700', textDecoration:'none', borderBottom:'1px solid rgba(207,233,230,0.4)' }}>{data.authority}</a> in {data.year}.
      </p>
      ...
      <div style={{ display:'flex', gap:'14px', justifyContent:'center', flexWrap:'wrap' }}>
        <a href="/contact" className="cta-primary">Get Free Assessment</a>
    ...
    <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
      ...
      <div style={{ borderRadius:'18px', padding:'26px 28px', marginBottom:'20px', background:'rgba(193,68,55,0.07)', border:'1px solid rgba(193,68,55,0.25)' }}>
        <h3 style={{ display:'flex', alignItems:'center', gap:'10px', fontSize:'1rem', fontWeight:'800', color:'#a5362a', margin:'0 0 10px', fontFamily:SF }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#a5362a" strokeWidth="1.8"/><path d="M12 9v4M12 17h.01" stroke="#a5362a" strokeWidth="1.8" strokeLinecap="round"/></svg>
      ...
      <div style={{ borderRadius:'18px', padding:'30px 28px', marginBottom:'70px', background:'linear-gradient(135deg, #2d7a7a 0%, #3a9494 100%)' }}>
        <h3 style={{ fontSize:'1.15rem', fontWeight:'800', color:'#fff', margin:'0 0 20px', fontFamily:SF }}>How iSeeWaves Supports {data.code} Compliance</h3>
...
/* ──────────────────────────────────────────────────────────
   BOTTOM CTA — centered, flat dark teal (matches hero tone)
   ────────────────────────────────────────────────────────── */
const BottomCTA = () => (
  <section style={{ padding:'70px 30px', textAlign:'center', background:'linear-gradient(135deg, #0c2326 0%, #163d3f 100%)', fontFamily:SF }}>
    ...
    <a href="/contact" className="cta-primary">Start Free Assessment</a>
  </section>
);

/* ──────────────────────────────────────────────────────────
   FOOTER — identical to CyberRegulations.jsx
   ────────────────────────────────────────────────────────── */
