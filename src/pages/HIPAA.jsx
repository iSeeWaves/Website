...
const data = {
  ...
  authorityUrl: 'https://www.hhs.gov/hipaa',
  year: '1996',
  description: "To protect sensitive patient health information from being disclosed without the patient's consent or knowledge.",

  whatIs: "HIPAA is a US federal law that establishes national standards to protect individuals' medical records and other personal health information (PHI).",
  roleImportance: 'It partitions compliance obligations between Covered Entities (healthcare providers, health plans, clearinghouses) and Business Associates (third-party vendors handling PHI), requiring mandatory Business Associate Agreements (BAAs) to extend liability down the supply chain. The HITECH Act of 2009 significantly strengthened HIPAA by making Business Associates directly and independently liable for violations, and introduced a tiered civil monetary penalty structure scaled by culpability—from unknowing violations to willful neglect.',
...
const GlobalStyles = () => (
  <style>{`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hipaa-nav-link { text-decoration:none; color:#F4F7EC; font-size:16px; font-weight:bold; transition:color 0.3s; padding-bottom:3px; }
    .hipaa-nav-link:hover { color:#fff; border-bottom:2px solid #fff; }

    .hipaa-footer-link { color:#b8d4d6; text-decoration:none; font-size:15px; display:inline-block; transition:all 0.3s ease; }
    .hipaa-footer-link:hover { color:#f3f7ec; transform:translateX(8px); }
    ...
    .hipaa-info-card:hover { transform:translateY(-3px); border-color:rgba(48,111,116,0.4); box-shadow:0 14px 32px rgba(48,111,116,0.12); }
    ...
    .hipaa-cta-primary:hover { transform:translateY(-2px); box-shadow:0 12px 30px rgba(48,111,116,0.55); }
    ...
    .hipaa-cta-secondary:hover { background:rgba(255,255,255,0.14); border-color:#74C3BA; transform:translateY(-2px); }
  `}</style>
);
      ...
      <ul style={{ display:'flex', listStyle:'none', margin:0, padding:0 }}>
        {[
          ...
          <li key={label} style={{ marginRight:'20px' }}>
            <a href={href} className="hipaa-nav-link">{label}</a>
          </li>
        ))}
      </ul>
    </nav>
  ...
  <section style={{ marginTop:'90px', position:'relative', overflow:'hidden', fontFamily:SF, background:'linear-gradient(180deg,#08191c 0%,#0c2326 60%,#0a1f22 100%)', padding:'90px 30px 80px' }}>
    ...
    <div style={{ position:'relative', zIndex:2, maxWidth:'720px', margin:'0 auto', textAlign:'center', animation:'heroFadeUp 0.6s ease both' }}>
      ...
      <div style={{ display:'flex', gap:'14px', justifyContent:'center', flexWrap:'wrap' }}>
        <a href="/contact" className="hipaa-cta-primary">Get Free Assessment</a>
        <a href="/contact" className="hipaa-cta-secondary">Talk to an Expert</a>
      </div>
    </div>
  </section>
);
...
const BottomCTA = () => (
  <section style={{ padding:'70px 30px', textAlign:'center', background:'linear-gradient(135deg,#0c2326 0%,#163d3f 100%)', fontFamily:SF }}>
    ...
    <a href="/contact" className="hipaa-cta-primary">Start Free Assessment</a>
  </section>
);
...
const FooterSection = ({ title, links }) => (
  <div>
    ...
    <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'12px' }}>
      {links.map(link => (
        <li key={link.label}><a href={link.href} className="hipaa-footer-link">{link.label}</a></li>
      ))}
    </ul>
  </div>
);
