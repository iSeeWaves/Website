...
const data = {
  code: 'PCFP Pakistan',
  title: 'Pakistan Cloud First Policy',
  authority: 'Ministry of Information Technology & Telecommunication (MoITT)',
  authorityUrl: 'https://moitt.gov.pk',
  year: '2022',
  description: 'To accelerate digital transformation by mandating cloud adoption across federal government agencies, ensuring data sovereignty, security, and operational efficiency.',

  whatIs: 'The Pakistan Cloud First Policy (PCFP) is a government directive that requires all federal ministries and departments to prioritize cloud-based solutions when procuring new IT infrastructure or upgrading existing systems.',
  roleImportance: 'It establishes a framework for secure cloud adoption, defines data classification requirements, mandates cybersecurity controls, and ensures that sensitive government data remains within approved cloud environments.',

  benefits: [
    'Accelerated digital transformation of government services',
    'Reduced IT infrastructure costs through shared cloud resources',
    'Enhanced data security and resilience through standardized controls',
    'Improved interoperability between government departments',
    'Support for national data sovereignty and localization goals',
  ],
  rules: [
    'Cloud-first approach mandatory for all new federal IT procurements',
    'Data must be classified (Public, Internal, Confidential, Restricted) before migration',
    'Only approved Cloud Service Providers (CSPs) may be used for sensitive data',
    'Agencies must conduct a Cloud Readiness Assessment before migration',
    'Incident reporting to IGNITE / NITB within defined timelines',
  ],

  whoMustComply: [
    'All federal ministries and attached departments',
    'Autonomous bodies and statutory organizations under federal government',
    'State-owned enterprises adopting new IT systems',
    'IT vendors and Cloud Service Providers supplying to federal agencies',
  ],
  complianceRequirements: [
    'Cloud Readiness Assessment (CRA) submission to NITB',
    'Data classification and tagging of all government data assets',
    'Use of IGNITE-approved or NITB-vetted Cloud Service Providers',
    'Implementation of Identity & Access Management (IAM) controls',
    'Regular security audits and compliance reporting to MoITT',
  ],

  penalties: 'Non-compliant agencies may face suspension of IT procurement approvals, audit findings escalated to the Auditor General of Pakistan, and disciplinary action under PECA 2016 and relevant government service rules for responsible officers.',

  howWeSupport: [
    'Cloud Readiness Assessment (CRA) facilitation',
    'Data classification and tagging implementation',
    'IAM and access control architecture design',
    'Cloud security posture management (CSPM)',
    'Vendor assessment for NITB-approved CSP selection',
...
const GlobalStyles = () => (
  <style>{`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .pcfp-pk-nav-link { text-decoration:none; color:#F4F7EC; font-size:16px; font-weight:bold; transition:color 0.3s; padding-bottom:3px; }
    .pcfp-pk-nav-link:hover { color:#fff; border-bottom:2px solid #fff; }

    .pcfp-pk-footer-link { color:#b8d4d6; text-decoration:none; font-size:15px; display:inline-block; transition:all 0.3s ease; }
    .pcfp-pk-footer-link:hover { color:#f3f7ec; transform:translateX(8px); }
    ...
    .pcfp-pk-info-card:hover { transform:translateY(-3px); border-color:rgba(48,111,116,0.4); box-shadow:0 14px 32px rgba(48,111,116,0.12); }
    ...
    .pcfp-pk-cta-primary:hover { transform:translateY(-2px); box-shadow:0 12px 30px rgba(48,111,116,0.55); }

    .pcfp-pk-cta-secondary {
      display:inline-flex; align-items:center; gap:10px;
      padding:14px 30px; border-radius:50px;
    ...
    .pcfp-pk-cta-secondary:hover { background:rgba(255,255,255,0.14); border-color:#74C3BA; transform:translateY(-2px); }
  `}</style>
);
      ...
      <ul style={{ display:'flex', listStyle:'none', margin:0, padding:0 }}>
        {[
          ...
          <li key={label} style={{ marginRight:'20px' }}>
            <a href={href} className="pcfp-pk-nav-link">{label}</a>
          </li>
        ))}
      </ul>
    </nav>
  ...
  <section style={{ marginTop:'90px', position:'relative', overflow:'hidden', fontFamily:SF, background:'linear-gradient(180deg,#08191c 0%,#0c2326 60%,#0a1f22 100%)', padding:'90px 30px 80px' }}>
    ...
    <div style={{ position:'relative', zIndex:2, maxWidth:'720px', margin:'0 auto', textAlign:'center', animation:'heroFadeUp 0.6s ease both' }}>
      ...
      <div style={{ display:'flex', gap:'14px', justifyContent:'center', flexWrap:'wrap' }}>
        <a href="/contact" className="pcfp-pk-cta-primary">Get Free Assessment</a>
        <a href="/contact" className="pcfp-pk-cta-secondary">Talk to an Expert</a>
      </div>
    </div>
  </section>
);
...
const InfoCard = ({ title, body, items, bullet }) => (
  <div className="pcfp-pk-info-card">
...
const BottomCTA = () => (
  <section style={{ padding:'70px 30px', textAlign:'center', background:'linear-gradient(135deg,#0c2326 0%,#163d3f 100%)', fontFamily:SF }}>
    ...
    <p style={{ fontSize:'15px', color:'rgba(255,255,255,0.55)', maxWidth:'520px', margin:'0 auto 30px', lineHeight:1.7 }}>
      Don't let compliance complexities slow down your operations. Partner with iSeeWaves to ensure continuous adherence and robust cloud security.
    </p>
    <a href="/contact" className="pcfp-pk-cta-primary">Start Free Assessment</a>
  </section>
);
...
const FooterSection = ({ title, links }) => (
  <div>
    ...
    <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'12px' }}>
      {links.map(link => (
        <li key={link.label}><a href={link.href} className="pcfp-pk-footer-link">{link.label}</a></li>
      ))}
    </ul>
  </div>
);
...
export default PCFPPakistanPage;